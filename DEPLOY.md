...
## Данные админки: что сохраняется после git push / redeploy

| Что вы меняете в админке | Где хранится | После нового коммита и деплоя |
|--------------------------|--------------|-------------------------------|
| Товары, цены, категории, баннеры (текст, ссылки) | **PostgreSQL** (`watta-sushi-db`) | **Остаются** — база не сбрасывается при деплое |
| Заказы, пользователи, настройки сайта | **PostgreSQL** | **Остаются** |
| Фото/видео, загруженные через админку | Папка **uploads** на сервере API | **Остаются**, если на Render включён **Persistent Disk** (план Starter и выше) |

**Важно:** `git push` и redeploy **не удаляют** данные в базе. Seed запускается только если БД **пустая** (0 категорий или 0 пользователей).

### Чтобы картинки и видео не пропадали на Render

На бесплатном плане диск сервиса **временный** — после перезапуска/деплоя файлы в `uploads` могут исчезнуть, хотя в БД останутся пути `/uploads/...`.

1. Render Dashboard → **watta-sushi-backend** → **Disks** → **Add disk** (нужен план **Starter** или выше).
2. **Mount path:** `/var/data`
3. **Size:** 1 GB (или больше)
4. **Environment** → добавить: `UPLOAD_DIR` = `/var/data/uploads`
5. **Save** и **Manual Deploy** бэкенда.

В логах при старте API должно быть: `📁 Uploads directory: /var/data/uploads (UPLOAD_DIR)`.

Локально файлы лежат в `web/public/uploads/` и тоже сохраняются между перезапусками.
