...
export function IngredientLocaleFields({ value, onChange, labels, langMeta }: Props) {
  ...
  return (
    <div className="admin-ing-locale space-y-3">
      <div>
        <p className="text-xs font-bold uppercase tracking-wide text-watta-action/80">{labels.sectionTitle}</p>
        <p className="mt-0.5 text-[11px] leading-snug text-watta-action/60">{labels.hint}</p>
      </div>
        ...
        {langMeta.map((meta) => {
          ...
          return (
            <label key={meta.suffix} className="admin-ing-locale__field block">
              <span className="mb-1 flex items-center gap-1.5 text-[11px] font-bold text-watta-action/75">
                <span
                  className={`inline-flex min-w-[1.75rem] justify-center rounded-md px-1.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wide ${
                    filled
                      ? 'bg-watta-action/12 text-watta-action'
              ...
              <input
                ...
                className="admin-ing-locale__input w-full rounded-xl border border-watta-action/15 bg-white px-3 py-2.5 text-sm outline-none transition focus:border-watta-action focus:ring-2 focus:ring-watta-action/15"
      ...
      {previewName ? (
        <div className="admin-ing-locale__preview rounded-xl border border-watta-action/12 bg-gradient-to-br from-[#f6faf8] to-white px-3 py-2.5">
          <p className="text-[10px] font-bold uppercase tracking-wider text-watta-action/55">{labels.previewTitle}</p>
          <div className="mt-2 flex flex-wrap gap-1.5">
            {INGREDIENT_LOCALE_SUFFIXES.map((suffix) => {
              ...
              return (
                <span
                  key={suffix}
                  className="inline-flex max-w-full items-center gap-1 rounded-full border border-watta-action/12 bg-white px-2 py-0.5 text-[10px] font-semibold text-[#0f241e] shadow-sm"
                >
                  <span className="text-[9px] font-extrabold uppercase text-watta-action/50">{suffix}</span>
