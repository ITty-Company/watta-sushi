...
export default function AdminView({ onBack }: AdminViewProps) {
  // Добавили вкладку 'promos' и 'cities'
  const [activeTab, setActiveTab] = useState<'orders' | 'products' | 'promos' | 'cities'>('orders')
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (activeTab === 'orders') {
        ...
        const [prodRes, catRes, citiesRes] = await Promise.all([
        ...
        // Загрузка промокодов
        const res = await fetch('/api/promo', { headers })
        if (res.ok) setPromos(await res.json())
        ...
        // Загрузка городов
        const res = await fetch('/api/cities/all', { headers })
  ...
  const handleCreateCity = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/cities', {
        ...
        body: JSON.stringify({ name: newCityName, name_nl: newCityNameNl || newCityName })
      })
      if (res.ok) {
        setNewCityName('')
        setNewCityNameNl('')
        ...
        const error = await res.json()
        alert(error.message || 'Ошибка создания города')
      }
    } catch (e) {
      console.error(e)
      alert('Ошибка сети')
  ...
  const handleUpdateCity = async (id: number, name: string, name_nl: string, isActive: boolean) => {
    try {
      const res = await fetch(`/api/cities/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, name_nl, isActive })
      })
      if (res.ok) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
          ...
          <div className="flex gap-6 md:gap-10 mb-8 ml-4 overflow-x-auto pb-2">
            ...
            <button 
              ...
              className={`text-[20px] md:text-[24px] font-bold tracking-tight transition-colors whitespace-nowrap ${
                activeTab === 'cities' ? 'text-[#165044]' : 'text-[#7C7C7C]'
              }`}
            >
              🏙️ Города
            </button>
          ...
          {activeTab === 'cities' && (
            <div className="flex flex-col gap-8">
              ...
              <div className="bg-white rounded-[25px] p-8 shadow-sm">
                <h2 className="text-xl font-bold text-[#155044] mb-4 flex items-center gap-2">
                  <MapPin /> Добавить новый город
                </h2>
                <form onSubmit={handleCreateCity} className="flex flex-col md:flex-row gap-4">
                  <input 
                    type="text" 
                    placeholder="Название города (RU)"
                    value={newCityName}
                    onChange={e => setNewCityName(e.target.value)}
                    className="flex-1 p-4 bg-[#F3F4F6] rounded-[15px] outline-none font-bold"
                    required
                  />
                  <input 
                    type="text" 
                    placeholder="Название города (NL)"
                    value={newCityNameNl}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {cities.map(city => (
                  <div key={city.id} className="bg-white rounded-[20px] p-6 shadow-sm flex flex-col gap-4 border border-gray-100">
                    {editingCityId === city.id ? (
                      <div className="flex flex-col gap-3">
                        <input
                          type="text"
                          defaultValue={city.name}
                          className="p-3 bg-[#F3F4F6] rounded-[10px] outline-none"
                          id={`city-name-${city.id}`}
                        />
                        <input
                          type="text"
                          defaultValue={city.name_nl || city.name}
                          className="p-3 bg-[#F3F4F6] rounded-[10px] outline-none"
                          id={`city-name-nl-${city.id}`}
                        />
                        <div className="flex gap-2">
                          <button
                            onClick={() => {
                              const nameInput = document.getElementById(`city-name-${city.id}`) as HTMLInputElement
                              const nameNlInput = document.getElementById(`city-name-nl-${city.id}`) as HTMLInputElement
                              handleUpdateCity(city.id, nameInput.value, nameNlInput.value, city.isActive)
                            }}
                            className="flex-1 px-4 py-2 bg-[#155044] text-white rounded-[10px] hover:bg-[#103d34] transition"
                          >
                            Сохранить
                          </button>
                          <button
                            onClick={() => setEditingCityId(null)}
                            className="px-4 py-2 bg-gray-200 rounded-[10px] hover:bg-gray-300 transition"
                          >
                            Отмена
                          </button>
                        </div>
                      ...
                      <>
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-[#F3F4F6] rounded-full flex items-center justify-center text-[#155044]">
                            <MapPin size={20} />
                          </div>
                          <div className="flex-1">
                            <h3 className="font-bold text-lg">{city.name}</h3>
                            {city.name_nl && city.name_nl !== city.name && (
                              <p className="text-sm text-gray-500">{city.name_nl}</p>
                            )}
                          </div>
                          <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                            city.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'
                          }`}>
            ...
            <form onSubmit={handleSubmitProduct} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto p-3 bg-[#F3F4F6] rounded-[10px]">
                    {cities.map(city => (
                      <label
                        key={city.id}
