...
export default function AuthView({ onBack, onLoginSuccess }: AuthViewProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // УСПЕХ: Сервер создал юзера и вернул userId. Ждем код.
        setUserId(data.userId) 
        // alert(`(DEV MODE) Ваш код: см. консоль сервера`) // Можно раскомментировать для удобства
        setIsVerifying(true) // Переключаем экран на ввод кода
      } else {
        alert(data.message || 'Ошибка регистрации')
      }
    } catch (err) {
  ...
  // 2. ВЕРИФИКАЦИЯ (Этап 2: Проверка кода)
  const handleVerify = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!userId) {
        alert('Ошибка: ID пользователя потерян. Попробуйте снова.')
    ...
    try {
      const res = await fetch('/api/auth/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // Отправляем ID юзера и введенный код
        body: JSON.stringify({ userId: userId, code: verificationCode })
      })
      const data = await res.json()
      
      if (res.ok) {
        // Код верный -> Сразу логиним юзера
        if (typeof window !== 'undefined') {
          localStorage.setItem('token', data.token)
          localStorage.setItem('currentUser', JSON.stringify(data.user))
        }
        onLoginSuccess() // Закрываем окно, обновляем шапку
      } else {
        alert(data.message || 'Неверный код')
      }
    } catch (err) {
