...
//       alert(t?.auth?.passwords_mismatch || 'Пароли не совпадают')
//       return
//     }
    
//     // 1. Отправляем данные регистрации
...
//       setIsVerifying(true) // Переключаем на экран ввода кода
//     } else {
...
//         <h2 className="auth-title-web">Подтверждение</h2>
//         <p className="text-center text-gray-500 mb-4">Введите код из письма</p>
//         <form onSubmit={handleVerify} className="auth-form-web">
//           <input 
//             type="text" 
//             placeholder="Код (6 цифр)" 
//             value={verificationCode}
//             onChange={(e) => setVerificationCode(e.target.value)}
//             className="auth-input-web text-center text-2xl tracking-widest"
//             required
//           />
//           <button type="submit" className="auth-submit-btn-web">Подтвердить</button>
//           <button type="button" onClick={() => setIsVerifying(false)} className="text-sm text-gray-500 mt-2">Назад</button>
//         </form>
//       </div>
//     </div>
//   )
...
export default function AuthView({ onBack, onLoginSuccess }: AuthViewProps) {
  ...
  // Добавили confirmPassword в состояние формы
  const [formData, setFormData] = useState({
    name: '',
    email: '',
    password: '',
  ...
  // Состояния для подтверждения кода
  const [isVerifying, setIsVerifying] = useState(false)
  const [verificationCode, setVerificationCode] = useState('')
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // Берем значения из состояния
    const { email, password, confirmPassword, name, phone } = formData

    if (password !== confirmPassword) {
      // ИСПРАВЛЕНИЕ: Используем (t?.auth as any), чтобы TypeScript не ругался на отсутствующее поле
      const msg = (t?.auth as any)?.passwords_mismatch || 'Пароли не совпадают'
      alert(msg)
      return
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    // Если это регистрация, вызываем отдельную функцию
    if (isRegister) {
    ...
    return (
      <div className="h-screen font-sans flex flex-col items-center justify-center relative overflow-hidden bg-gray-900">
        <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md mx-4">
          <h2 className="text-2xl font-bold text-center mb-2 text-gray-800">Подтверждение</h2>
          <p className="text-center text-gray-500 mb-6">Введите код из письма, отправленного на {formData.email}</p>
          <form onSubmit={handleVerify} className="flex flex-col gap-4">
            <input 
              ...
              value={verificationCode}
              onChange={(e) => setVerificationCode(e.target.value)}
              className="text-center text-3xl tracking-[0.5em] font-bold border-2 border-gray-200 rounded-xl p-4 focus:border-[#145142] outline-none transition-colors"
              required
              maxLength={6}
            />
            <button 
              type="submit" 
              className="bg-[#145142] text-white font-bold py-4 rounded-xl hover:bg-[#0f3d32] transition-colors disabled:opacity-50"
              disabled={isLoading}
            >
