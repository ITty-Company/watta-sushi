...
export default function ProfileAddressesFlow({
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-10 text-[#64748b]" role="status">
  ...
  return (
    <div className="watta-profile-address-flow">
      {isEmpty ? (
        <div className="watta-profile-address-flow__empty" role="status">
          <ProfileAddressEmptyScene title={cp.addrEmptyTitle} subtitle={cp.addrEmptySub} />
        </div>
      ) : (
        <section className="watta-profile-address-flow__saved" aria-label={cp.addrSavedList}>
          <div className="watta-profile-address-flow__saved-head">
            <h3 className="watta-profile-address-flow__saved-title">{cp.addrSavedList}</h3>
            <span className="watta-profile-address-flow__saved-count">{addresses.length}</span>
          </div>
      ...
      <div className="watta-profile-address-flow__add">
        {!isEmpty && !addOpen ? (
          <button
            type="button"
