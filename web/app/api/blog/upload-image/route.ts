...
/** Multipart upload обкладинки блогу — проксі на backend. */
export async function POST(request: NextRequest) {
  const apiBase = backendBaseUrl()
  const url = `${apiBase}/api/blog/upload-image`
  ...
  try {
    ...
    if (err?.name === 'AbortError') {
      return NextResponse.json({ message: 'Таймаут завантаження фото' }, { status: 504 })
    }
    if (isBackendConnectionError(fetchError)) {
      return NextResponse.json({ message: backendUnreachableMessage(apiBase) }, { status: 503 })
    }
    return NextResponse.json({ message: 'Помилка завантаження фото' }, { status: 502 })
  }
}
